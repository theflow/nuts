...
function Nuts(opts) {
    ...
    this.router.get('/update/channel/:channel/:platform/:version', this.onUpdateForChannel);
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        if (versions.length === 1) {
            releaseNotes = notes.merge(versions, { includeTag: false });
        } else {
            releaseNotes = notes.merge(versions.slice(0, -1), { includeTag: false });
        }
...
Nuts.prototype.onUpdateForChannel = function(req, res, next) {
    var that = this;
    var fullUrl = getFullUrl(req);
    var platform = req.params.platform;
    var tag = req.params.version;
    ...
    .then(function(versions) {
        ...
        var releaseNotes = notes.merge(versions, { includeTag: false });

        res.status(200).send({
            "url": urljoin(fullUrl, '/../../../../../', '/download/channel/'+channel+'/'+platform+'?filetype='+filetype),
            "name": latest.tag,
            "notes": releaseNotes,
            "pub_date": latest.published_at.toISOString()
        });
